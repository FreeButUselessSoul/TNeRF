...
class BlenderDatasetPatched(Dataset):
    def __init__(self, root_dir, split='train', img_wh=(800, 800),
                 perturbation=[], sample_wh=100):
        self.root_dir = root_dir
        self.split = split
        assert img_wh[0] == img_wh[1], 'image width must equal image height!'
        self.img_wh = img_wh
        self.define_transforms()
        self.img2patch = ImgToPatch(FlexGridRaySampler(sample_wh),sample_wh)
    ...
    def read_meta(self):
        ...
        if self.split == 'train': # create buffer of all rays and rgb data
            self.all_rays_o = dict() # id: tensor of (3, H, W)
            self.all_rays_d = dict()
            self.near_far = dict()
            self.all_rgbs = dict() # id: tensor of (3, H, W)
            for t, frame in enumerate(self.meta['frames']):
                ...
                self.all_rgbs[t] = img # (3, h, w)
                
                rays_o, rays_d = get_rays(self.directions, c2w, expand=False) # both (3, h, w)
                self.all_rays_o[t] = rays_o
                self.all_rays_d[t] = rays_d
                self.near_far[t] = torch.Tensor([self.near, self.far])
                # self.all_rays[t] = torch.cat([rays_o, rays_d,
                #                              self.near*torch.ones((h, w, 1)),
                #                              self.far*torch.ones((h, w, 1))],
                #                              2) # (h, w, 8)
    ...
    def __getitem__(self, idx):
        if self.split == 'train': # use data in the buffers
            ...
            rgb,rays_o,rays_d = self.img2patch(rgb,rays_o,rays_d) # (h*w,3)
            rays = torch.cat([rays_o,rays_d,
                            self.near_far[idx]*torch.ones((len(rays_o),2)),1])
            sample = {'rays': rays,
                      'ts': idx * torch.ones((len(rays_o),1), dtype=torch.long),
                      'rgbs': self.all_rgbs[idx]}
            ...
            img = img.resize(self.img_wh, Image.LANCZOS)
            img = self.transform(img) # (4, H, W)
            valid_mask = (img[-1]>0) # (H, W) valid color area
            img = img[:3] * img[-1] + (1 - img[-1]) # blend A to RGB
