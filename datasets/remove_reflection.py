...
if __name__=='__main__':
    ...
    for i in range(len(image_paths)):
        ...
        inp = T.ToTensor()(img_ori).unsqueeze(0)
        hypercolumn = vgg(inp)
        _, C, H, W = inp.shape
        ...
        img_out = img_out.cpu().detach().squeeze().permute(1,2,0).numpy()
        img_out = np.uint8(img_out*255)
        # import ipdb;ipdb.set_trace()
        Image.fromarray(img_out).save(os.path.join(args.out_dir,'images/'+image_path.split('/')[-1]),quality=100,subsampling=0)

    otherfiles=['database.db','poses_bounds.npy']
    for filename in otherfiles:
        shutil.copyfile(os.path.join(args.tar_dir,filename), os.path.join(args.out_dir,filename))
