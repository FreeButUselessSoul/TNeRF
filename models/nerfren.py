...
class NeRF(nn.Module):
    def __init__(self, typ,
        ...
        self.reflection_fraction = nn.Sequential(nn.Linear(W, 1), nn.Sigmoid())
    ...
    def forward(self, x, sigma_only=False, output_transient=True):
        ...
        if sigma_only:
            input_xyz = x
        elif output_transient and self.in_channels_t>0:
        ...
        reflection_fraction = self.reflection_fraction(xyz_)
        if sigma_only:
            return static_sigma
