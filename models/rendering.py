...
def render_rays(models,
    ...
    def inference(results, model, xyz, z_vals, test_time=False, **kwargs):
        ...
        if typ=='coarse' and test_time:
            ...
            if output_transient and t_embedded is not None:
        ...
        if output_transient:
            Reversed_transmittance = torch.flip(torch.cumprod(torch.flip(alphas_shifted[:,:-1],[1]),-1),[1])
            Reversed_weights = static_alphas * Reversed_transmittance
        ...
        if output_transient:
            results['bdc'] = torch.abs(Reversed_weights-static_weights)
    ...
    if N_importance > 0: # sample points for fine model
        ...
        if model.encode_appearance:
            if 'a_embedded' in kwargs:
                a_embedded = embeddings['a'](torch.zeros_like(ts))
