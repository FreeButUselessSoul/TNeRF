...
class NeRFWarp(nn.Module):
    def __init__(self, typ,
        ...
        self.static_rgb = nn.Sequential(nn.Linear(W//2+in_channels_f, W//2),nn.ELU(inplace=True),
        ...
        if self.encode_transient:
            ...
            #                             nn.Linear(W+in_channels_t+self.in_channels_f, W//2), nn.ReLU(True),
            ...
            self.transient_rgb = nn.Sequential(nn.Linear(W//2, W//2), nn.ReLU(inplace=True),
                                            nn.Linear(W//2, 3), nn.Sigmoid())
            self.transient_beta = nn.Sequential(nn.Linear(W//2, 1),nn.Sigmoid()) # weight map
    ...
    def forward(self, x, sigma_only=False, output_transient=True):
        ...
        transient_beta = self.transient_beta(dir_encoding) # (B, 1)
