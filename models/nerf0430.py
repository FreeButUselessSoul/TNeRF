...
class NeRF(nn.Module):
    def __init__(self, typ,
        ...
        self.dir_encoding = nn.Sequential(
                        nn.Linear(W+in_channels_dir+self.in_channels_a+64, W), nn.ReLU(True),
                        nn.Linear(W, W//2), nn.ReLU(True))

        # static output layers
        self.static_sigma = nn.Sequential(nn.Linear(W, 1), nn.Softplus())
        self.codeB_simplify = nn.Sequential(nn.Linear(in_channels_f, 64),nn.ReLU(inplace=True))
        self.static_rgb = nn.Sequential(nn.Linear(W//2, W//2),nn.ELU(inplace=True),
                                        nn.Linear(W//2, 3),nn.Sigmoid())

        if self.encode_transient:
            # transient encoding layers
            self.transient_encoding = nn.Sequential(
                                        nn.Linear(W+in_channels_t, W), nn.ELU(inplace=True),
                                        nn.Linear(W, W), nn.ELU(inplace=True),
                                        nn.Linear(W, W//2), nn.ELU(inplace=True),
                                        nn.Linear(W//2, W//2), nn.ELU(inplace=True))
            # transient output layers
            self.transient_sigma = nn.Sequential(nn.Linear(W//2, W//2), nn.Softplus(),
                                nn.Linear(W//2, 1), nn.Softplus())
            self.transient_rgb = nn.Sequential(nn.Linear(W//2, W//2), nn.ReLU(inplace=True),
                                            nn.Linear(W//2, 3), nn.Sigmoid())
            self.transient_beta = nn.Sequential(nn.Linear(in_channels_t+W//2, 1),nn.Sigmoid()) # weight map
    ...
    def forward(self, x, sigma_only=False, output_transient=True):
        ...
        code_B2 = self.codeB_simplify(code_B)
        dir_encoding_input = torch.cat([xyz_encoding_final, input_dir_a, code_B2], 1)
        dir_encoding = self.dir_encoding(dir_encoding_input)
        
        # static_input = torch.cat([dir_encoding, code_B2], 1)
        ...
        # transient_encoding_input = torch.cat([xyz_encoding_final, input_t, code_R], 1)
        ...
        transient_beta_input = torch.cat([input_t,dir_encoding], 1)
        transient_sigma = self.transient_sigma(transient_encoding) # (B, 1)
        transient_rgb = self.transient_rgb(transient_encoding) # (B, 3)
        transient_beta = self.transient_beta(transient_beta_input) # (B, 1)

        transient = torch.cat([transient_rgb, transient_sigma,transient_beta
                               ], 1) # (B, 5)
