...
class NeRF(nn.Module):
    ...
    def forward(self, x, sigma_only=False, output_transient=True):
        ...
        """
        if sigma_only:
            input_xyz, input_features = torch.split(x,[self.in_channels_xyz,self.in_channels_f],dim=-1)
        elif output_transient:
            input_xyz, input_features, input_dir_a, input_t = \
                torch.split(x, [self.in_channels_xyz, self.in_channels_f,
                                self.in_channels_dir+self.in_channels_a,
                                self.in_channels_t], dim=-1)
        else:
            input_xyz, input_features, input_dir_a = \
                torch.split(x, [self.in_channels_xyz, self.in_channels_f,
        ...
        static_input = torch.cat([dir_encoding, input_features], 1)
